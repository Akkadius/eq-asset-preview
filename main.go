...
type config struct {
	...
	spriteSize           uint    // exact size eg 15,30,50 [ ]
	spritePercentageSize float32 // percentage of original .4 = 40%
}
...
func main() {
	...
	spriteSheets := []config{
		...
		// legacy spell icons
		{
			directory:       "./assets/spell_icons_legacy",
			iconNamePrefix:  "spell-",
			iconNameSuffix:  "-12",
			spriteImageFile: "./assets/sprites/spell-icons-legacy-12.png",
			spriteCssFile:   "./assets/sprites/spell-icons-legacy-12.css",
			spriteSize:      12,
		},
		{
			directory:       "./assets/spell_icons_legacy",
			iconNamePrefix:  "spell-",
			iconNameSuffix:  "-20",
			spriteImageFile: "./assets/sprites/spell-icons-legacy-20.png",
			spriteCssFile:   "./assets/sprites/spell-icons-legacy-20.css",
			spriteSize:      20,
		},
		{
			directory:       "./assets/spell_icons_legacy",
			iconNamePrefix:  "spell-",
			iconNameSuffix:  "-30",
			spriteImageFile: "./assets/sprites/spell-icons-legacy-30.png",
			spriteCssFile:   "./assets/sprites/spell-icons-legacy-30.css",
			spriteSize:      30,
		},
		{
			directory:       "./assets/spell_icons_legacy",
			iconNamePrefix:  "spell-",
			iconNameSuffix:  "-40",
			spriteImageFile: "./assets/sprites/spell-icons-legacy-40.png",
			spriteCssFile:   "./assets/sprites/spell-icons-legacy-40.css",
			spriteSize:      40,
		},
		{
		...
		},
		{
			...
			iconNamePrefix:       "race-models-",
			iconNameSuffix:       "-sm",
			spriteImageFile:      "./assets/sprites/race-models-sm.png",
			spriteCssFile:        "./assets/sprites/race-models-sm.css",
			spritePercentageSize: .50,
		},
		{
			directory:            "./assets/faces",
			iconNamePrefix:       "faces-",
			spriteImageFile:      "./assets/sprites/faces.png",
		...
		{
			directory:            "./assets/client-versions",
			iconNamePrefix:       "client-version-sm-",
			spriteImageFile:      "./assets/sprites/client-versions-sm.png",
			spriteCssFile:        "./assets/sprites/client-versions-sm.css",
			spritePercentageSize: .25,
		},
	}
	...
	for _, c := range spriteSheets {
		...
		for _, file := range dir {
			...
			if c.spriteSize > 0 {
				imgDecoded = resize.Resize(c.spriteSize, c.spriteSize, imgDecoded, resize.NearestNeighbor)
			}

			// if we are making the original image a percentage of the original
			if c.spritePercentageSize > 0 {
				w := float32(imgDecoded.Bounds().Dx()) * c.spritePercentageSize
				h := float32(imgDecoded.Bounds().Dy()) * c.spritePercentageSize
				imgDecoded = resize.Resize(uint(w), uint(h), imgDecoded, resize.NearestNeighbor)
			}
